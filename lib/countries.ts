/**
 * Comprehensive ISO 3166-1 alpha-2 country list with currencies.
 *
 * COUNTRY_LIST — full list of ~250 countries sorted alphabetically by name
 * COUNTRY_CURRENCIES — maps country code → primary ISO 4217 currency code
 * ALL_COUNTRY_LABELS — Record<string, string> for use wherever COUNTRY_LABELS was used
 */
...
/** code → name lookup (replaces the old 11-country COUNTRY_LABELS) */
export const ALL_COUNTRY_LABELS: Record<string, string> = Object.fromEntries(
  COUNTRY_LIST.map((c) => [c.code, c.name])
);

/** code → currency lookup */
export const COUNTRY_CURRENCIES: Record<string, string> = Object.fromEntries(
  COUNTRY_LIST.map((c) => [c.code, c.currency])
);

/** Default benefit categories for countries without a CountryConfig */
...
/**
 * Get the currency for a country code. Falls back to USD if unknown.
 */
...
/**
 * Get display name for a country code. Falls back to the code itself if unknown.
 */
export function getCountryName(code: string): string {
  return ALL_COUNTRY_LABELS[code] || code;
}
