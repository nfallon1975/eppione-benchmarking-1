...
export default function ComplianceDashboard() {
  ...
  useEffect(() => {
    ...
    if (session.user?.role !== "CLIENT" && session.user?.role !== "BROKER" && session.user?.role !== "ADMIN") {
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/compliance/results?country=${selectedCountry}`);
      if (res.ok) {
        const data = await res.json();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-500">
            Check your benefits against country regulatory requirements and statutory limits
          </p>
        </div>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Click &quot;Run Check&quot; to check your benefits against {COUNTRY_LABELS[selectedCountry] || selectedCountry} requirements.
            </p>
            <p className="mt-2 text-xs text-slate-400">
              Note: Compliance rules may not yet be configured for all countries. If no results appear after running a check, it means no country-specific requirements have been set up yet.
            </p>
          </CardContent>
        </Card>
      )}
          ...
          <TabsContent value="limits">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Tax relief caps, contribution limits, and other statutory thresholds for {COUNTRY_LABELS[selectedCountry] || selectedCountry}
                </CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                <Table>
                    ...
                    {limits.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={7} className="py-8 text-center text-sm text-slate-500">
                          No statutory limits data available for this country.
                        </TableCell>
                      </TableRow>
                    )}
                    {limits.map((limit) => (
