...
import { FileText, CheckCircle2, AlertTriangle, Info, ChevronDown, ExternalLink, ArrowLeft } from "lucide-react";
...
export default function AdminBenchmarkingPage() {
  ...
  const companyNameParam = searchParams.get("name") || "Client";
  ...
  useEffect(() => {
    async function loadCompany() {
      try {
        const res = await fetch(`/api/companies?companyId=${encodeURIComponent(companyId)}`);
        if (!res.ok) return;
        ...
        setCompanyName(data.name || companyNameParam);
  ...
  const fetchBenchmarks = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const params = new URLSearchParams({ country, currency, companyId });
  ...
  }, [country, industry, currency, companyId]);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900">
            Benchmarking — {companyName}
          </h1>
          <p className="mt-1 text-slate-500">
            Viewing benchmarking data for this client
          </p>
        </div>
      </div>
        ...
        <Tabs defaultValue="overview">
          ...
          {isMultiCountry && benchmarkData.dataQuality !== "reference" && (
            <TabsContent value="cross-country">
              <BenchmarkCrossCountryTab currency={currency} companyId={companyId} />
            </TabsContent>
          )}
