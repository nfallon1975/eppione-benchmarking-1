...
export default function AdminBenefitsPage() {
  ...
  const companyName = searchParams.get("name") || companyId;
  ...
  async function fetchBenefits() {
    try {
      const res = await fetch(`/api/benefits?companyId=${companyId}`);
      if (res.ok) {
  ...
  useEffect(() => {
    if (status !== "loading" && session?.user) {
      fetchBenefits();
      fetch(`/api/company/countries?companyId=${companyId}`)
        .then((r) => r.json())
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // POST needs ?companyId for admin override; PUT uses role-based bypass
      const url = editingId
        ? `/api/benefits/${editingId}`
        : `/api/benefits?companyId=${companyId}`;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-900">
              Benefits &mdash; {decodeURIComponent(companyName)}
            </h1>
            <p className="mt-1 text-slate-500">
              Manage benefit entries for this client
            </p>
          </div>
        </div>
          ...
          {benefits.length === 0 ? (
            <div className="py-12 text-center">
              <p className="text-sm text-slate-500">
                No benefit entries yet. Add the first benefit for this client.
              </p>
