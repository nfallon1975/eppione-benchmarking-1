...
export default function AdminSubmissionsPage() {
  ...
  async function fetchSubmissions() {
    try {
      ...
      const res = await fetch(`/api/admin/submissions${qs ? `?${qs}` : ""}`);
      if (res.ok) {
  ...
  async function toggleExpand(companyId: string) {
    ...
    try {
      const res = await fetch(`/api/admin/submissions/${companyId}`);
      if (res.ok) {
        const data = await res.json();
        setDetailData(data);
      }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-900">Company Submissions</h1>
        <p className="mt-1 text-slate-500">
          View all completed survey submissions with full answers and contact information
        </p>
      </div>
